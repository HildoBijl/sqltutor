...
export function Theory() {
  return <Page>
    ...
    <Section title="Examples of query languages">
      ...
      <Par>Suppose that we want to get a list of companies with more than 2000 employees. In the <Term>SQL</Term> query language (the most commonly used query language) that would be done through</Par>
      <Par><pre><code>{`SELECT company_name
FROM companies
WHERE num_employees > 2000
`}</code></pre></Par>
      <Par>In <Term>Datalog</Term> (a more modern and up-and-coming query language) this would be done with</Par>
      <Par><pre><code>{`largeCompanies(name) :- companies(_, name, _, _, num_employees, _), num_employees > 2000.
?- largeCompanies(name).`}</code></pre></Par>
      ...
      <Par><pre><code>largeCompanies ← ∏<sub>company_name</sub>(σ<sub>num_employees &gt; 2000</sub>(companies))</code></pre></Par>
  ...
  { company_name: 1, _id: 0 }
    ...
    <Section title="Two branches of query languages">
      ...
      <List items={[
        ...
        <>The <Term>Data Manipulation Language</Term> (DML) focuses on working with data: adding/updating records, retrieving the right data, etcetera.</>,
      ]} />
