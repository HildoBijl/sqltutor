...
export function Summary() {
  return <Page>
    <Section>
      <Par>To <Term>filter rows</Term> using <Term>SQL</Term> we add the <SQLDisplay inline>WHERE</SQLDisplay> keyword to our query, followed by a condition like <SQLDisplay inline>column_name = some_value</SQLDisplay>. We can set up a comparison for equals <SQLDisplay inline>=</SQLDisplay>, unequals <SQLDisplay inline>{`<>`}</SQLDisplay>, larger than <SQLDisplay inline>{`>`}</SQLDisplay>, smaller than <SQLDisplay inline>{`<`}</SQLDisplay>, larger-or-equal <SQLDisplay inline>{`>=`}</SQLDisplay>, or smaller-or-equal <SQLDisplay inline>{`<=`}</SQLDisplay>. For larger/smaller than, text is compared lexicographically and dates/times are compared time-wise, with earlier being smaller.</Par>
