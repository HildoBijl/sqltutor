...
export function Summary() {
  return <Page>
    <Section>
      ...
      <List items={[
        <>An <Term>attribute</Term> is a combination of a name and a domain. If an attribute is named {`A`}, its domain is denoted by {`dom(A)`}. The <Term>domain</Term> is the set of all possible values which this attribute may have.</>,
      ...
      <Par>Because relation instances are sets, it means they have no ordering and cannot have duplicates. If there would be duplicates, the excess tuples are implicitly removed.</Par>
    </Section>
  </Page>;
}
