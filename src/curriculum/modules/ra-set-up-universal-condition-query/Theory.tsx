...
import { Page, Section, Par, List, Warning, Term, Em, M, BM, RA, IRA } from '@/components';
...
export function Theory() {
  return <Page>
    ...
    <Section title="Flip the request: use a double negative">
      <Par>Let's study the employee contracts from the <Term>emp_data</Term> relation. These contracts can have a variety of statuses.</Par>
      ...
      <Par>To start, we apply some tricks we used before.</Par>
      <List items={[
        <>We <Term>manually execute the request</Term>. To find the respective employees, we must run a checklist for each employee. "Have they been on sick leave? Have they been on paid leave?" And so forth. At the end, we should check if the checklist is fully ticked off. If so, the employee has had all statuses. Whatever relational algebra query we end up writing will have to do something similar.</>,
    ...
    <Section title="Gather data: set up an entity-requirement table">
      <Par>When starting the script, it helps to set up two supporting lists.</Par>
      <List sx={{ my: -1 }} itemSx={{ my: 1 }} contentSpacing={1} items={[
        <>
          <Par>The <Term>entity list</Term> contains (references to) all the entities that we want to run a checklist for. For us, that is the list of all employee IDs.</Par>
        ...
        <>
          <Par>The <Term>requirement list</Term> contains (references to) all the checks that we need to run for said entities. For us, that is that the list of all possible contract statuses.</Par>
      ...
      <Par>Once we know the entities and the requirements, we set up the <Term>entity-requirement table</Term>. This is the relation that has all combinations (entity, requirement) that hold <Em>according to the given data</Em>. For us, that is the list of statuses that the employees have held.</Par>
      ...
      <Par>The entity-requirement table is a relation with two columns. However, it is very helpful to imagine it differently: as a 2-dimensional table, with the entities listed vertically and the requirements listed horizontally. Each field contains a checkmark if the given combination is present in our data.</Par>
      <EntityRequirementTable />
      <Par>Having this table in mind makes all the subsequent steps a lot easier.</Par>
    </Section>

    <Section title="Apply the conditions: combine/flip and squash the combination table">
      <Par>Once we have the combination table, we apply the given conditions to it. In our example case, we want to find if an employee has had <Em>all</Em> statuses. This is difficult, but we could also do the opposite: see if there is a status that each employee has <Em>not</Em> had. To do so, we flip the combination table. This is done in two steps.</Par>
      <List sx={{ my: -1 }} itemSx={{ my: 1 }} contentSpacing={1} items={[
        <>
          <Par>Set up the <Term>Cartesian product</Term> of the entity list and the requirement list. This denotes the table with <Em>all</Em> fields checked.</Par>
          <Box><EntityRequirementTable all={true} /></Box>
        </>,
        <>
          <Par>Remove from this the entries in the entity-requirement table. This gives us the list of statuses which employees have <Em>not</Em> had.</Par>
          <Box><EntityRequirementTable flip={true} /></Box>
        </>,
      ]} />
      ...
      <Par>Once we have applied the conditions, we squash the entity-requirement table into a list: we take the projection with respect to the entities. This gives us the employees for which there is a status they have <Em>not</Em> had.</Par>
      ...
      <Par>This is our final result. Note that, since our query had a double negative, our full procedure must also have two flips (set differences).</Par>
    </Section>

    <Section title="Use the shortcut: division">
      ...
      <Par>The request of "finding all entities for which a certain requirement holds" is a common use case though. Because of that, an operator has been defined to make this procedure easier: the <Term>division operator</Term> <M>\div</M>.</Par>
      ...
      <FigureDivisionTable />
      <Par>The result of this division will be all employee IDs for which <Em>every</Em> status is present in the left relation.</Par>
      ...
      <Par>Note that the above formula is exactly the procedure that we have followed in the script above.</Par>
      ...
      <Par>This is both a lot shorter and a lot clearer than what we had before! The division operator can hence be a very powerful short-cut in universal condition queries.</Par>
      <Warning>
        <Par sx={{ mb: 1 }}>You can only apply the division operator when the requirement list is a <Term>fixed set</Term>: it does not depend on the entity that is being compared. For our example that is the case: we are checking the same statuses for every employee.</Par>
        <Par>But now let's adjust our problem. What if we want to find all employees who have had all the statuses that <Em>their direct supervisor</Em> has had? In this case, the requirement list (which statuses they should have had) varies per employee! Since the requirement list is not a fixed set, the division operator will not work here. In such a case, we have no choice but follow the steps described above: set up the entity-requirement table and subsequently apply the given conditions to it.</Par>
      </Warning>
    </Section>
  </Page>;
}
...
export function EntityRequirementTable({ all = false, flip = false, scale = 0.8 }) {
    ...
    <Element scale={scale} anchor={[-1, -1]}>
      <TableContainer ref={tRef} component={Paper} sx={{ width: tableWidth }}>
        <Table size="small">
          <TableHead>
            <TableRow>
              <TableCell /> {/* top-left empty corner */}
