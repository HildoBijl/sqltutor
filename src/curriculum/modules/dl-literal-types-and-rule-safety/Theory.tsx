...
export function Theory() {
  return <Page>
    ...
    <Section title="Unbound variables causing infinitely large outputs">
      ...
      <Par>The problem here is that the variable <IDL>id</IDL> is an <Term>unbound variable</Term>: it is not constrained by any positive literal. When this is the case, we run the risk of infinitely large outputs. It should most certainly be avoided. Luckily the solution we showed earlier is relatively simple: we bound the <IDL>id</IDL> variable through a positive literal <IDL>employeeId(id)</IDL>. In other words, we specify something it <Em>should</Em> be.</Par>
    </Section>
    ...
    <Section title="Rule safety">
      <Par>Based on the above problems, we can come up with the idea of <Term>rule safety</Term>. We call a Datalog rule <Term>safe</Term> if <Em>every</Em> variable in the rule appears in at least one positive non-arithmetic predicate in the rule body.</Par>
