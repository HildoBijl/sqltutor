...
export function Theory() {
  return <Page>
    ...
    <Section title="Filter: add the join condition">
      ...
      <Par>In Datalog, if we ever use the same variable name, then this variable must have the <Em>same</Em> value everywhere where it is used. We use this idea to shorten the above rule: we use the <Em>same variable name</Em> for the manager ID and the employee ID, denoting both as <IDL>mid</IDL>. (Any name will do, as long as it's equal.)</Par>
      <FigureExampleDLQuery query={<>departmentWithManager(did, dn, b, ne, eid, fn, ln, p, e, a, c, hd, cs) :- department(did, dn, mid, b, ne), employee(mid, fn, ln, p, e, a, c, hd, cs).</>} actualQuery="SELECT d.*, e.first_name, e.last_name, e.phone, e.email, e.address, e.city, e.hire_date, e.current_salary FROM departments d JOIN employees e ON d.manager_id=e.e_id" tableWidth={1400} below />
      <Info>As usual, Datalog will look for all sets of possible values for <Em>all</Em> the variables we used (including <IDL>mid</IDL>) such that <Em>all</Em> literals hold true. So <Em>both</Em> <IDL>department(did, dn, mid, b, ne)</IDL> and <IDL>employee(mid, fn, ln, p, e, a, c, hd, cs)</IDL> must hold true for our set of variables. Because we used the variable <IDL>mid</IDL> in both, the value of <IDL>mid</IDL> must match in both predicates! That's why this short-cut implicitly requires the manager ID to be equal to the employee ID.</Info>
    </Section>

    <Section title="Projection: select required arguments">
      <Par>Obiously we don't always need all arguments. Suppose that, for each department, we only want the manager's name and contact info. We could then turn all other variables into anonymous variables. This simplifies the rule further.</Par>
      ...
      <Info>Because Datalog does not use argument names, there is no such thing as a natural join. Luckily the non-natural join is easy enough to set up.</Info>
    </Section>
  </Page>;
}
