...
export function Theory() {
  return <Page>
    ...
    <Section title="Join a relation with itself">
      ...
      <Par>The solution to this reference problem is to use a <Term>table rename</Term>. The <Term>rename operator</Term> <M>\rho</M> can (next to attribute renames) also be used to temporarily give a relation within a query a different name. We could for instance set up our full query as</Par>
      ...
      <Par>Once we have this table, we can compare every person's job contracts with every one of their other job contracts. Our goal is to find two job contracts with different positions. So we add this condition to the filter.</Par>
      ...
      <Par>Note that these are all possible combinations of two contracts of the <Em>same person</Em> but with <Em>different</Em> positions. This is almost what we want. The final step is extracting the <Em>people</Em> having these contracts, which is done through a projection. Extracting the employee IDs gives the final result.</Par>
    ...
    <Section title="Use the rename operator">
      ...
      <List items={[
        ...
        <>As a <Term>table rename</Term> <IRA>ρ<sub>e2</sub>(emp_data)</IRA>. This changes the name of the relation given to it. It is only used to support proper referencing in the rest of the query, as it doesn't do anything else.</>,
        <>As a joint <Term>table/attribute rename</Term> <IRA>ρ<sub>e2, e_id→manager_id, salary→contract_salary</sub>(emp_data)</IRA>. This combines the above two functions into one. Note that the new relation name is given first without an arrow, and any number of attribute renames is given afterwards using the <Term>arrow notation</Term>.</>,
      ]} />
