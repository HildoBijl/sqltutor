...
SELECT * FROM vendor_chain;`} tableWidth={260} />
    ...
    <Section title="Use a recursion counter">
      ...
      <Quote>We say that v received money from b in <IDL>m + 1</IDL> steps, if there is some other person x for which v received money from x in n steps, and x sold something to b.</Quote>
      ...
      <Warning>You may have already learned about Datalog rule safety. The literal <IDL>n = m + 1</IDL> causes the rule to be unsafe: <IDL>n</IDL> does not originate from any existing predicate. That explains the infinitely large outcomes.</Warning>
      <Info>Some more strict Datalog enginers don't allow literals like <IDL>n = m + 1</IDL>. Instead, they use a <Term>successor predicate</Term> <IDL>succ(m, n)</IDL> which defines <IDL>n</IDL> to be one larger than (i.e., the successor of) <IDL>m</IDL>. So instead of <IDL>n = m + 1</IDL> we'd then have to use <IDL>succ(m, n)</IDL>.</Info>
