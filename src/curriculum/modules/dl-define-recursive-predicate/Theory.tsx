...
SELECT * FROM vendor_chain;`} tableWidth={260} />
    ...
    <Section title="Use a recursion counter">
      ...
      <DL>receivedMoneyInSteps(v, b, m) :- receivedMoneyInSteps(v, x, n), transaction(_, x, b, _, _, _, _, _), m = n + 1.</DL>
      <Par>In words, this can be phrased as</Par>
      <Quote>We say that v received money from b in (n+1) steps, if there is some other person x for which v received money from x in n steps, and x sold something to b.</Quote>
      ...
      <Warning>You may have already learned about Datalog rule safety. The literal <IDL>m = n+1</IDL> causes the rule to be unsafe: <IDL>m</IDL> does not originate from any existing predicate. That explains the infinitely large outcomes.</Warning>
      <Info>Some more strict Datalog enginers don't allow literals like <IDL>m = n+1</IDL>. Instead, they use a <Term>successor predicate</Term> <IDL>succ(n, m)</IDL> which defines <IDL>m</IDL> to be one larger than (i.e., the successor of) <IDL>n</IDL>. So instead of <IDL>m = n+1</IDL> we'd then have to use <IDL>succ(n, m)</IDL> then.</Info>
      <Par>Tackling the problem of cycles is really tricky. There's a few approaches that can do so, and that will give the <Em>least</Em> amount of steps for which B can be reached from A. However, these approaches require techniques we haven't discussed so far: think of tracking which links we already visited, or using recursive aggregation within Datalog. Because it's a bit too advanced, we won't discuss those methods further for now. Whenever we add a counter, we'll simply assume there are no cycles.</Par>
    </Section>
  </Page>;
}
