...
export function Theory() {
  return <Page>
    <Section>
      <Par>We know the definition of rule safety in Datalog: a Datalog rule is safe if <Em>every</Em> variable in the rule appears in at least one positive non-arithmetic predicate in the rule body. Let's study how we can check if a rule actually is safe.</Par>
    </Section>
    ...
    <Section title="Step 2: check that all variables are bound">
      <Par>Once we have a list of variables, we want to check if they're all in a positive non-arithmetic literal. In practice, this means they should all directly come from some predicate. Arithmetic literals like <IDL>salary &gt; b/20</IDL> and negative literals like <IDL>not allocation(mid, did)</IDL> are not important here and should be ignored.</Par>
      <Par>For our example, we can check that all variables appear as argument at either <IDL>department</IDL> or <IDL>employee</IDL>. When we do, we check all of them off except for the last one. Apparently <IDL>salary</IDL> is left unbound!</Par>
      <Info>It's also useful to check what kind of result the unbound variable will have. If it's in the output, we'll get infinitely many outputs. If it's not, we'll have an ineffective literal. In the example case, <IDL>salary</IDL> is not in the head of the rule, which means that we have an ineffective literal. The condition <IDL>salary &gt; b/20</IDL> will not have any effect.</Info>
      <Par>When we look closely at the Datalog rule, we can see what happened. By accident, we called the current salary <IDL>cs</IDL> within the predicate, but <IDL>salary</IDL> within the value comparison. It's a simple programmer's mistake, but checking for safety helped us find it. By fixing this, we indeed wind up with a safe rule.</Par>
