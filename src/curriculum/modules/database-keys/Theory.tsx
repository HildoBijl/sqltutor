...
export function Theory() {
  return <Page>
    ...
    <Section title="Primary key: a chosen candidate key">
      ...
      <Par>The primary key is indicated in the schema through an underline. <List items={[<><RelationName>employees</RelationName> (<PrimaryKey>e_id</PrimaryKey>, first_name, last_name, phone, email, address, city, hire_date, current_salary)</>]} /> In the case where the primary key consists of multiple attributes (like for instance <ISQL>{`{first_name, last_name}`}</ISQL>) then all the respective attributes are underlined. <List items={[<><RelationName>employees</RelationName> (e_id, <PrimaryKey>first_name</PrimaryKey>, <PrimaryKey>last_name</PrimaryKey>, phone, email, address, city, hire_date, current_salary)</>]} /> Note that by defining a primary key, we require these attributes (their combination) to be unique.</Par>
    </Section>
  </Page>;
}
