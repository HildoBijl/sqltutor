...
SELECT * FROM vendor_chain;`;
  ...
  return <Page>
    ...
    <Section title="The problem: current methods don't allow indefinite looping through chains">
      ...
      <Par>But now let's try to follow the money. If a person A sells something to a person B, and a person B sells something to a person C, then person A will have indirectly received money from person C. We call this a <Term>chain</Term>. And in this chain, some of person C's (possibly digital) banknotes will likely have ended up in person A's hands!</Par>
      <Par>We can add these two-step chains to our table: we can ask the database for all combinations of people (A, C) such that there is a person B for whom it holds that A sold to B <Em>and</Em> B sold to C.</Par>
      <FigureSingleTable query={q3} title="Received money in two steps" tableWidth={260} tableScale={0.8} />
      <Par>Pretty much all query languages allow this. And we can also increase the complexity of our query to find all three-step chains. And then we can merge all tables to get an overview of all one-step, two-step and three-step chains in one table. It's all pretty useful.</Par>
    ...
    <Section title="The solution: a query that refers to itself">
      ...
      <Info>The query above refers to itself! To be precise: to define whether or not someone "received money" from someone else, we actually used the words "received money from". This makes the definition self-referencing (i.e., recursive)</Info>
    ...
    <Section title="Evaluating recursive queries: the fixed-point algorithm">
      ...
      <List items={[
        ...
        <>
          ...
          <Par>Repeat this until <M>\Delta</M> ends up being empty. If <M>\Delta</M> ever <Em>is</Em> empty, and we have not found any new cases during one iteration, then we won't find any more cases in future iterations either. We're done searching.</Par>
        </>,
      ]} />
      ...
      <Par>This shows how the DBMS will keep searching for new cases until there's none left to be found. That's exactly what happens on every recursive query.</Par>
    </Section>
  </Page>;
}
